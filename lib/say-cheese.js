...
//console.log("Cheeses! I am inside the mousetrap!");
  SayCheese = function SayCheese(element, options) {
    ...
    this.options = {
      ...
      audio: false,
      width: 320,
      format: 'blob'       // Do this by default, can select 'blob' instead
    };
...
//         Due to patchy support for .toBlob, get it as a dataURL and convert (with polyfill)
          snapList.push(window.dataURLtoBlob(snapshot.toDataURL('image/png')));
        }
        if (that.options.format === 'blob') {
          snapshot.toBlob(function(blob) {console.log('Saved blob');snapList.push(blob)},'image/png');
        }
      });
...
// Pass back the dataURL
      if (this.options.format === 'dataurl') {
        this.trigger('snapshot', [window.dataURLtoBlob(snapshot.toDataURL('image/png'))]);
      } 
// Pass back a blob
